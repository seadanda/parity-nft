...
import { addToWhitelist, isEmailWhitelisted, createVerificationCode, verifyCode, createSession, validateSession } from '../src/lib/db';
...
async function testEmailFlow() {
  ...
  try {
    await addToWhitelist(TEST_EMAIL, 'Test User', 'Testing', 'Test account');
  ...
  const whitelisted = await isEmailWhitelisted(TEST_EMAIL);
  ...
  const code = await createVerificationCode(TEST_EMAIL, '127.0.0.1', 'test-script');
  ...
  const isValid = await verifyCode(TEST_EMAIL, code);
  ...
  const sessionToken = await createSession(TEST_EMAIL, '127.0.0.1', 'test-script');
